document.addEventListener("DOMContentLoaded", function () {
    ...
    let startX = 0;
    let startY = 0;       // New: to record vertical start position
    let currentDelta = 0;
    const dragThreshold = 10; // in px
    let ignoreHorizontal = false; // New flag to ignore horizontal swipe if vertical dominates

    function touchStartHandler(e) {
    isDragging = true;
    startX = e.touches ? e.touches[0].clientX : e.clientX;
    startY = e.touches ? e.touches[0].clientY : e.clientY; // Record vertical position
    ignoreHorizontal = false; // Reset flag
    slides.forEach(slide => {
        slide.style.transition = "none";
    });
    }
    ...
    let currentY = e.touches ? e.touches[0].clientY : e.clientY; // New: current vertical position
    currentDelta = currentX - startX;
    let verticalDelta = currentY - startY;
    // If vertical movement is dominant (and above threshold), ignore horizontal swipe
    ...
    if (!isDragging) return;
    isDragging = false;
    // Re-enable transitions using our helper
    slides.forEach((slide) => {
        slide.style.transition = getTransition();
    });
    // If vertical movement dominated, revert to current slide without sliding
    if (ignoreHorizontal) {
        slides.forEach((slide, i) => {
        slide.style.transform = `translateX(${(i - currentIndex) * 100}%)`;
        });
    ...
    // Determine if swipe was enough to change slides
